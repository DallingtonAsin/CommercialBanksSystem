...
class ImportRetirementSaving implements ToModel, WithHeadingRow
{
   /**
    * @param array $row
    *
   ...
   public function getMemberdata($accountno)
   {
   	...
   	$id = User::where('acc_noR', $accountno)
              	->value('id');

   	$acc_name = User::where('acc_noR', $accountno)
   	         ->value('acc_name');
